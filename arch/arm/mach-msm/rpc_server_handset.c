...
#include <linux/wakelock.h> //spd bug 316718
...
enum hs_event {
	...
	HS_EVNT_MAX = 0x7FFFFFFF,		/* Force enum to be an 32-bit number */
};
...
static struct wake_lock headset_wake_lock; //spd bug 316718
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
    ...
    printk(KERN_ERR "%s: keycode =  %d, key_parm = %d\n", __func__, key_code, key_parm);
	...
	case SW_HEADPHONE_INSERT_W_MIC:
		...
		input_report_switch(hs->ipdev, SW_MICROPHONE_INSERT,
							hs->mic_on);
		wake_lock_timeout(&headset_wake_lock, 2 * HZ); //spd bug 316718
		update_state();
		break;
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, hs);
      wake_lock_init(&headset_wake_lock, WAKE_LOCK_SUSPEND, "headset"); //spd bug 316718
