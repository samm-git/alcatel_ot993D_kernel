...
/*wqf for bug 125957*/
static void  hac_pa_enable(bool on )
{
	printk("==JRD===hac_pa_enable() : %d!\n", on);
	
       if (PM_GPIO_MODE_OUTPUT != pmic_gpio_get_direction(PM_GPIO_10))
       {
		pmic_gpio_direction_output(PM_GPIO_10);
       }
	...
	if(on){
		...
		pmic_set_speaker_gain(SPKR_GAIN_PLUS08DB);
		pmic_spkr_en_left_chan(1);
#else
		pmic_gpio_set_value(PM_GPIO_10, 1);
#endif		
	}else{
#if 0	
...
#else
		pmic_gpio_set_value(PM_GPIO_10, 0);	
#endif		
	}
}
...
//wqf for bug 125957
                if((spkr_param & 0x04) != 0)            //hac enable
                         hac_pa_enable(true);
                 else
                        hac_pa_enable(false);
                rc = 0;
