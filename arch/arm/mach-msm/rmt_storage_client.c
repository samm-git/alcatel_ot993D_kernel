...
extern int entry_msm_pm_restart;
			    ...
			    unsigned long arg)
{
	...
	case RMT_STORAGE_WAIT_FOR_REQ:
		...
		WARN_ON(kevent == NULL);
		memcpy(&rmc->event,&kevent->event,sizeof(struct rmt_storage_event));
...
void wait_for_rmt_storage_completion(void)
{
	struct msm_rpc_client *rpc_client;
	struct rmt_storage_send_sts status;
	int ret = 0;
	if(atomic_read(&rmc->wcount) !=0 ){
		pr_err("%s: Event %d still in process while shutdown!Waiting for 5s here\n",__func__,rmc->event.id);
		current->state = TASK_INTERRUPTIBLE;
		schedule_timeout(5 * HZ);
	}
	if(atomic_read(&rmc->wcount) !=0 ){
		pr_err("%s: Event %d process timeout in 5s while shutdown\n",__func__,rmc->event.id);
		status.err_code = -EIO;
		status.data = rmc->event.usr_data;
		status.handle = rmc->event.handle;
		status.xfer_dir = rmc->event.id;
		rpc_client = rmt_storage_get_rpc_client(status.handle);
		if (rpc_client){
			ret = msm_rpc_client_req2(rpc_client,
				RMT_STORAGE_OP_FINISH_PROC,
