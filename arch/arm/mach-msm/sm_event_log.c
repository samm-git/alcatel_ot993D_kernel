...
static unsigned long long time_ticks;
...
static __always_inline void log_irq_info(unsigned long ip,  unsigned int flags)
{
	...
	cur_ticks = readl(MSM_GPT_BASE + TIMER_COUNT_VAL);
	delta = cur_ticks - last_ticks;
	last_ticks = cur_ticks;
	time_ticks += delta;

	g_track_index = (g_track_index + 1) & (TRACK_BUF_SIZE - 1);
	g_track_irq_buf[g_track_index].cycles = time_ticks;
	g_track_irq_buf[g_track_index].spinlock = 0;

	g_track_irq_buf[g_track_index].ip = ip;
	g_track_irq_buf[g_track_index].flags = flags;
...
static int32_t sm_add_log_event(uint32_t event_id, uint32_t param1, int param2, void *data, uint32_t data_len)
{
	...
	if (likely(event_id & SM_IRQ_ONOFF_EVENT)) {
		if (unlikely(event_id & SM_IRQ_ONOFF_SPINLOCK)) {
			g_track_irq_buf[param2].spinlock = param1;
		} else
...
static int32_t sm_log_event_init (void)
{
	...
	input.extension.len = 1;
	input.extension.data[0] = (uint32_t)__virt_to_phys(&g_track_index);
	input.address = (uint32_t)__virt_to_phys(g_track_irq_buf);
