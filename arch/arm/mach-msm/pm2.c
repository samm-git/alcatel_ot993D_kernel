	...
	(bool from_idle, uint32_t sleep_delay, uint32_t sleep_limit)
{
	...
	int collapsed = 0;
#ifdef CONFIG_MSM_SM_EVENT
	uint64_t sclk_suspend_time, sclk_resume_time, sclk_period;
#endif
	int ret;
...
extern void wait_for_rmt_storage_completion(void);

static uint32_t restart_reason = 0x776655AA;

static void msm_pm_power_off(void)
{
	wait_for_rmt_storage_completion();
	msm_rpcrouter_close();
...
int entry_msm_pm_restart = 0;
static void msm_pm_restart(char str, const char *cmd)
{
	wait_for_rmt_storage_completion();
	entry_msm_pm_restart = 1;

	printk(KERN_ERR "msm_pm_restart++++++++ msm_rpcrouter_close\n");
	msm_rpcrouter_close();
	printk(KERN_ERR "msm_pm_restart++++++++ restart_reason 0x%x\n", restart_reason);
	msm_proc_comm(PCOM_RESET_CHIP, &restart_reason, 0);
	
	printk(KERN_ERR "msm_pm_restart--------\n");
