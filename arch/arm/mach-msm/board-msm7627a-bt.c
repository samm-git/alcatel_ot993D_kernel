...
{
	if (machine_is_msm7627a_qrd1() || machine_is_msm7627a_qrd3())
	{
		if (machine_is_msm7627a_qrd3() && hw_version_is(10, 0))
			gpio_bt_sys_rest_en = GPIO_BT_SYS_REST_EN_SKU3_1;
		else
			gpio_bt_sys_rest_en = msm_gpio_table_qrd[GPIO_BT_SYS_REST_EN_INDEX];
	}
}
...
static int fm_radio_setup(struct marimba_fm_platform_data *pdata)
{
	...
	if(machine_is_msm7627a_qrd3() || machine_is_msm7627a_qrd1())
		irqcfg = GPIO_CFG(msm_gpio_table_qrd[GPIO_FM_RDS_INT_INDEX], 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL,
			GPIO_CFG_2MA);
	else
...
static void fm_radio_shutdown(struct marimba_fm_platform_data *pdata)
{
	...
	uint32_t irqcfg;
	if(machine_is_msm7627a_qrd3() || machine_is_msm7627a_qrd1())
		irqcfg = GPIO_CFG(msm_gpio_table_qrd[GPIO_FM_RDS_INT_INDEX], 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_UP,
		GPIO_CFG_2MA);
	else
		irqcfg = GPIO_CFG(FM_GPIO, 0, GPIO_CFG_INPUT,
...
static int config_i2s(int mode)
{
	...
	if (mode == FM_I2S_ON) {
		if (machine_is_msm7x27a_surf() || machine_is_msm7625a_surf() || machine_is_msm7627a_qrd1() || machine_is_msm7627a_qrd3())
...
static int bluetooth_power(int on)
{
	...
	if (on) {
        ...
        if(machine_is_msm7627a_qrd1()&&!hw_version_ge(3,3)){
                rc = gpio_tlmm_config(GPIO_CFG(msm_gpio_table_qrd[GPIO_VDD33_EN_INDEX], 0,
                                GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL,
                                GPIO_CFG_2MA), GPIO_CFG_ENABLE);
                if (rc) {
                        pr_err("%s: gpio_tlmm_config for %d failed\n",
                                __func__, msm_gpio_table_qrd[GPIO_VDD33_EN_INDEX]);
						goto fail_gpio;
                }
                gpio_set_value_vote(msm_gpio_table_qrd[GPIO_VDD33_EN_INDEX],1,GPIO_VOTE_VDD33_BT,GPIO_VOTE_VDD33_BT_MASK);
        }
		rc = bt_set_gpio(on);
		if (rc) {
			pr_err("%s: bt_set_gpio = %d\n",
...
fail_gpio:
		if(machine_is_msm7627a_qrd1()&&!hw_version_ge(3,3)){
				gpio_set_value_vote(msm_gpio_table_qrd[GPIO_VDD33_EN_INDEX],0,GPIO_VOTE_VDD33_BT,GPIO_VOTE_VDD33_ALL_MASK);
		}
