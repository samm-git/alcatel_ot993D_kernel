...
#define AKM8975_DRDY_TIMEOUT	500
	...
	int *rbuf)
{
	...
	if (ready & ORI_DATA_READY) {
		input_report_abs(akm->input, ABS_RX, rbuf[0]);
		input_report_abs(akm->input, ABS_RY, rbuf[1]);
		input_report_abs(akm->input, ABS_RZ, rbuf[2]);
	...
	if (ready & MAG_DATA_READY) {
		...
		input_report_abs(akm->input, ABS_GAS, rbuf[4]);
	}
	...
	struct akm8975_data *akm, char const *buf, size_t count, int pos)
{
	...
	//printk(KERN_INFO "akm8975 delay[%d]=%d\n ",pos,d );
	mutex_unlock(&akm->val_mutex);
...
#endif
static ssize_t akm8975_bin_accel_write(struct file *filp,
	struct kobject *kobj,
...
static struct bin_attribute akm8975_bin_attributes[] = {__BIN_ATTR(accel, 0220, 6, NULL,
...
static void akm8975_early_suspend(struct early_suspend *handler)
{
	...
	AKECS_SetMode(s_akm, AK8975_MODE_POWERDOWN);
