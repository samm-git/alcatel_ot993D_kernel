...
extern int load_565rle_image(void);
#endif
			    ...
			    boolean op_enable)
{
	...
	struct msm_fb_panel_data *pdata = NULL;
	int ret = 0;
	static int called_first_time=0;
...
//add for bug222022
				if(!called_first_time){
					...
					bl_updated = 1;
					if(unset_bl_level)
						msm_fb_set_backlight(mfd,unset_bl_level);
					else if(mfd->bl_level)
						msm_fb_set_backlight(mfd,mfd->bl_level);					
				}
//end add for bug222022
				#endif
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	MSM_FB_INFO
	    ("FrameBuffer[%d] %dx%d size=%d bytes, bits_per_pixel=%d bits is registered successfully!\n",
	     mfd->index, fbi->var.xres, fbi->var.yres, fbi->fix.smem_len, fbi->var.bits_per_pixel);

#ifdef CONFIG_FB_MSM_LOGO
	if (!load_565rle_image()) {
		struct msm_fb_panel_data *pdata =
			(struct msm_fb_panel_data *)mfd->pdev->dev.platform_data;
...
#define MSMFB_SCREEN_ON 0x11222211
#define POWEROFF_CHARGING_ON_BACKLIGHT 39
extern int fastboot_power_off;
	...
	case MSMFB_SCREEN_ON:
		if (!mfd->bl_level && fastboot_power_off == 0) {
			msm_fb_set_backlight(mfd, POWEROFF_CHARGING_ON_BACKLIGHT);
		}
		break;
