...
static const char default_ctrl_regs[] = {
	0x7F,	/* 210HZ, PM-normal, xyz enable */
	0x00,	/* normal mode */
				...
				struct k3g_t *data, int total_read)
{
	...
	/*for test*/
	//printk("GYRO sensor data x = %8d , y = %8d , z = %8d \n", data->x, data->y, data->z);
	return 0;
}
...
static enum hrtimer_restart k3g_timer_func(struct hrtimer *timer)
{
	...
	if(likely(k3g_data->timer_enable))
			       ...
			       const struct i2c_device_id *devid)
{
	...
	gyro_debug("GYRO sensor ID is %d\n",ret);
	...
	if (data->client->irq >= 0) { /* interrupt */
		...
		gyro_debug("GYRO sensor enter the interrupt mode\n");
		...
		data->ctrl_regs[2] = 0x80; /* enable interrupt */

	} else { /* polling */
		u64 delay_ns;
		gyro_debug("GYRO sensor enter the polling mode\n");
		...
		gyro_debug("GYRO sensor data->entries is %d\n",data->entries);
