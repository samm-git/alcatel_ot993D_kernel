/*
 *  STMicroelectronics LIS3DH accelerometer sensor driver
 *
...
#ifdef LIS3DH_USE_FIFO
static int lis3dh_read_fifo_status(struct lis3dh_data *lis3dh_data)
{
	int fifo_status;
	fifo_status =
	    i2c_smbus_read_byte_data(lis3dh_data->client, FIFO_SRC_REG);
	if (fifo_status < 0) {
...
static int lis3dh_restart_fifo(struct lis3dh_data *lis3dh_data)
{
	int res = 0;
	res = i2c_smbus_write_byte_data(lis3dh_data->client,
	...
	res = i2c_smbus_write_byte_data(lis3dh_data->client,
					FIFO_CTRL_REG,
					FIFO_MODE | (lis3dh_data->entries - 1));
	if (res < 0)
				  ...
				  struct lis3dh_t *data, int total_read)
{
	...
	err =
	    i2c_smbus_read_i2c_block_data(client, reg, sizeof(acc_data),
					  acc_data);
...
/* currently we are not using the fifo */
#if 0
	res = lis3dh_read_fifo_status(lis3dh_data);
	lis3dh_data->drop_next_event = !res;
	if (res >= 31 - lis3dh_data->entries) {
		...
		return lis3dh_restart_fifo(lis3dh_data);
	}
#endif
#ifdef CONFIG_MACH_MSM7X27A_SKU3
	input_report_rel(lis3dh_data->input_dev, REL_RX, (-1) * data.x);
	input_report_rel(lis3dh_data->input_dev, REL_RY, (-1) * data.y);
...
{
	struct lis3dh_data *lis3dh_data =
	    container_of(timer, struct lis3dh_data, timer);
...
/*currently we are not using the fifo */
	...
	do {
		...
		if (res < 0)
			return;
timer_set:
		set_polling_delay(lis3dh_data, res);
	} while (!ktime_to_ns(lis3dh_data->polling_delay));
#endif

	if (likely(lis3dh_data->work_enable))
		hrtimer_start(&lis3dh_data->timer, lis3dh_data->polling_delay,
			      HRTIMER_MODE_REL);
}
...
#if 0
		/* reset fifo entries */
		err = lis3dh_restart_fifo(lis3dh_data);
#endif
		if (err < 0) {
			err = -EIO;
			goto turn_off;
...
turn_off:
	if (err < 0)
		i2c_smbus_write_byte_data(lis3dh_data->client,
					  CTRL_REG1, LIS3DH_ACC_DISABLE);
unlock:
				...
				size_t size)
{
	...
	if (odr_value != (lis3dh_data->ctrl_regs[0] & ODR_MASK)) {
		...
		res =
		    i2c_smbus_write_byte_data(lis3dh_data->client, CTRL_REG1,
					      ctrl);
	}
	...
	/* (re)start fifo */
	//lis3dh_restart_fifo(lis3dh_data);
