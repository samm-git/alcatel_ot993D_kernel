...
#define TS_WAKE_GPIO				94
#define TS_RESET_PMIC_GPIO			97
...
#else
#define TS_IRQ_GPIO					123
#define TS_WAKE_GPIO				122 
#define TS_RESET_PMIC_GPIO			5  
#define TS_I2C_ADDR					0x38
//#define FW_UPGRADE_ENABLE 1		/*Enable upgrade TP Firmware through i2c bus*/
//#define RESET_ENABLE 1		/*Enable RESET */
#define WAKE_ENABLE 1		/*Enable WAKE*/
...
#define KEY_Y_BOTTOM		864 //839 //wqf
...
#define REPORT_RELEASE_TP(a, b) { \
	input_report_abs(a->input, ABS_MT_TOUCH_MAJOR,0); \
}
...
#ifdef RESET_ENABLE
#if 0
static struct pm8058_gpio ft5306_reset = { 
    .direction      = PM_GPIO_DIR_OUT,
    .output_buffer  = PM_GPIO_OUT_BUF_CMOS,
    .output_value   = 1,
    .pull           = PM_GPIO_PULL_NO,
    .vin_sel        = PM_GPIO_VIN_L5,  /*set pmic gpio_7 to 2.85v*/
...
{
#include "FTS0094P430_CockTail_V18_20110914_ap-1.h"
};
...
static int ft5306_reset_config(void)
{
    ...
    ret = gpio_request((TS_RESET_PMIC_GPIO), "ft5306_reset");	
    if (ret){
...
#if 0						
    ret = pm8058_gpio_config(TS_RESET_PMIC_GPIO, &ft5306_reset);
    if (ret){
        pr_err("%s: FAIL pm8058_gpio_config(): ret=%d.\n",__func__, ret);
        return -EIO;//goto err_ven;
    }
#else
     gpio_tlmm_config(GPIO_CFG(TS_RESET_PMIC_GPIO, 0, GPIO_CFG_OUTPUT, 
				GPIO_CFG_PULL_UP, GPIO_CFG_8MA), GPIO_CFG_ENABLE);
     /*Pull up at init*/
     gpio_set_value(TS_RESET_PMIC_GPIO, 1);	
#endif
	return 0;
}
...
static void ft5306_reset_func(void)
{
	...
	gpio_set_value((TS_RESET_PMIC_GPIO), 0);
...
{
	int key_type = 0; 
	//printk("wqf------------report_key pressed %d\n", flag);	
...
static void work_func(struct work_struct *work)
{
	...
	//input_sync(data->input);
	       /** 
        * Bug#204706
        * When fastdown skip touchscreen report
        */
       if (fastboot_power_off == 0) {
               input_sync(data->input);
       }
