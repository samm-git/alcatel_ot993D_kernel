...
{
	struct rmi_sensor_device * rdev = container_of(dev, struct rmi_sensor_device, dev);
	struct rmi_sensor_driver * sensor = rdev->driver;
	struct rmi_function_info *functionInfo;
	int retval;
	unsigned short ctlreg = 0x0000;
	unsigned char regval = 0x00;

	list_for_each_entry(functionInfo, &sensor->functions, link) {
		if ((functionInfo->functionNum == 0x01)) {
			ctlreg = functionInfo->funcDescriptor.controlBaseAddr;
		}
	}

	retval = rmi_read(sensor, ctlreg, &regval);
	if (retval) {
		printk(KERN_ERR "%s: sensor read fail(retval =%d)!", __func__, retval);
		return retval;
	}
	retval = rmi_write(sensor, ctlreg, (regval & 0xfc) || 0x01);
	if (retval == 1) {
		return 0;
	}
	else if (retval == 0) {
		printk(KERN_ERR "%s: sensor suspend fail!", __func__);
		return -EINVAL;	/* TODO: What should this be? */
	}
	else {
		printk(KERN_ERR "%s: sensor suspend maybe fail!", __func__);
		return retval;
	}
}
...
static int rmi_sensor_resume(struct device *dev)
{
	...
	//enter normal mode
	retval = rmi_write(sensor, ctlreg, regval & 0xfc);
	if (retval == 1) {
		return 0;
	}
	else if (retval == 0) {
		printk(KERN_ERR "%s: sensor resume fail!", __func__);
		return -EINVAL;	/* TODO: What should this be? */
	}
	else {
		printk(KERN_ERR "%s: sensor resume maybe fail!", __func__);
		return retval;
	}
}

#ifdef CONFIG_HAS_EARLYSUSPEND
static void rmi_early_suspend(struct early_suspend *h)
{
	struct rmi_sensor_driver *driver = container_of(h, struct rmi_sensor_driver, early_suspend);

	rmi_sensor_suspend(&driver->sensor_device->dev, PMSG_SUSPEND);
}

static void rmi_late_resume(struct early_suspend *h)
{
	struct rmi_sensor_driver *driver = container_of(h, struct rmi_sensor_driver, early_suspend);

	rmi_sensor_resume(&driver->sensor_device->dev);
}
#endif
/*
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	driver->drv.suspend = NULL;
	driver->drv.resume = NULL;
	driver->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN;
	driver->early_suspend.suspend = rmi_early_suspend;
	driver->early_suspend.resume = rmi_late_resume;
	register_early_suspend(&driver->early_suspend);
#else
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&driver->early_suspend);
#endif
}
