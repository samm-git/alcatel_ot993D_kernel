...
unsigned int fastboot_usb_on;
extern int mipi_set_bl(int level);
extern void msm_keypad_bl_led_set_fastboot(enum led_brightness value);
...
//add by zhangmin pr241410 start
...
#define USB_CIGARCHARGER_CHG_CURRENT 500
static int usb_get_max_power(struct usb_info *ui)
{
	...
	if (suspended || !configured)
		return USB_CIGARCHARGER_CHG_CURRENT;
		//return 0;
...
static void usb_chg_detect_non_standard_wall_charger(struct work_struct *w)
{
	...
	if(temp != USB_CHG_TYPE__WALLCHARGER &&
		...
		atomic_set(&otg->chg_type, USB_CHG_TYPE__NON_STANDARD_WALLCHARGER);
		otg_set_power(ui->xceiv, 500);
	}
}
#endif
	...
	for (;;) {
		...
		case USB_STATE_ONLINE:
			...
			if (flags & USB_FLAG_VBUS_OFFLINE) {
				...
				if(ac_in_flag)
					{
					ac_in_flag = FALSE;	/*AC or USB charger removed*/
				...
				//	printk("fastboot_power_off +++ = %d\n",fastboot_power_off);
					mipi_set_bl(0);
					msm_keypad_bl_led_set_fastboot(LED_OFF);
					fastboot_power_off = 3;	
					fastboot_bklight_off = 1;
				//	printk("fastboot_power_off --- = %d\n",fastboot_power_off);
				} else {
					printk("fastboot_power_off error = %d\n",fastboot_power_off);
				}
				printk(KERN_INFO "AC or USB charger removed now!\n"); 	
		...
		case USB_STATE_OFFLINE:
			...
			if ((flags & USB_FLAG_VBUS_ONLINE) && _vbus) {
				...
				if(!ac_in_flag)
				{	
					ac_in_flag = TRUE;		/*AC or USB charger pluggedd in*/
					switch_set_state(&jrd_charger_sdev, ac_in_flag);
...
static int msm72k_probe(struct platform_device *pdev)
{
	...
	if (retval)
		{
		printk(KERN_ERR "register jrd_charger_sdev fail !\n");		
		return usb_free(ui, retval);
		}		
	printk(KERN_INFO "register jrd_charger_sdev success!\n");
#endif
	the_usb_info = ui;
	...
	if (retval) {
		...
		printk(KERN_INFO "unregister jrd_charger_sdev now!\n");
	#endif
...
int usb_gadget_unregister_driver(struct usb_gadget_driver *driver)
{
	...
	printk(KERN_INFO "send jrd_charger_sdev uevent=0 \n");
#endif	
	/* cancel pending ep0 transactions */
