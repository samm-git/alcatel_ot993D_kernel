...
static int msm_otg_set_power(struct otg_transceiver *xceiv, unsigned mA)
{
	...
	if (new_chg != curr_chg && pdata->chg_connected) {
		...
		// temporary use type USB_CHG_TYPE__SDP for
		// USB_CHG_TYPE__NON_STANDARD_WALLCHARGER due to mp couldn't support
		// USB_CHG_TYPE__NON_STANDARD_WALLCHARGER now.
		if(USB_CHG_TYPE__NON_STANDARD_WALLCHARGER == new_chg)
			new_chg = USB_CHG_TYPE__WALLCHARGER;
		pr_debug("Use USB_CHG_TYPE__SDP for"
			" USB_CHG_TYPE__NON_STANDARD_WALLCHARGER now!\n");
#endif
