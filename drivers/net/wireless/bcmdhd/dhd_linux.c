...
{
	int ret = NOTIFY_DONE;
//shaohua add, or wifi will send command before mmc resume
...
static int dhd_set_suspend(int value, dhd_pub_t *dhd)
{
	...
	int bcn_li_dtim = 3;
	uint roamvar = 1;
	char null_data[]="ff:ff:ff:ff:ff:ff:ff";
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
				...
				memset(iovbuf, 0, sizeof(iovbuf));                          //clear buf
                                bcm_mkiovar("send_nulldata", (char *)&null_data, 4, iovbuf, sizeof(iovbuf));
                                dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
			} else {
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	dhd_save_drvdata(dhdp);
	
	if(!dhd_download_fw_on_driverload){
		dhd_bcmsdh_start(dhdp,1);
		DHD_ERROR(("%s: Brcm SDIO first restart now\n", __FUNCTION__));
	}
#endif
