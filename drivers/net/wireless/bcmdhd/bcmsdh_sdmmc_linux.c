...
static struct mmc_host *brcm_mmc_host;
#endif
#endif
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2, 6, 39)) && defined(CONFIG_PM)
#if defined(SDIO_ISR_THREAD)
static int brcm_enable_sdio_irq(struct sdio_func *func, int enable)
{
	if (brcm_mmc_host&&brcm_mmc_host->ops &&
			brcm_mmc_host->ops->enable_sdio_irq) {
		brcm_mmc_host->ops->enable_sdio_irq(brcm_mmc_host, enable);
		return 0;
	}
...
static int bcmsdh_sdmmc_suspend(struct device *pdev)
{
	...
	printk("%s: @@@@BCM4330 enter suspend @@@, func->num=%d\n",__func__,func->num);
	sd_trace(("%s Enter\n", __FUNCTION__));
...
#elif defined(SDIO_ISR_THREAD)
	brcm_enable_sdio_irq(func,0);
#endif
...
#endif
	printk("%s: @@@@BCM4330 exit suspend @@@, func->num=%d\n",__func__,func->num);
	return ret;
}
...
static int bcmsdh_sdmmc_resume(struct device *pdev)
{
	...
	printk("%s: @@@@BCM4330 enter resume @@@, func->num=%d\n",__func__,func->num);
#if defined(OOB_INTR_ONLY)
	if ((func->num == 2) && dhd_os_check_if_up(bcmsdh_get_drvdata()))
		bcmsdh_oob_intr_set(1);
#elif defined(SDIO_ISR_THREAD)
	if ((func->num == 2) && dhd_os_check_if_up(bcmsdh_get_drvdata()))
		brcm_enable_sdio_irq(func,1);
#endif
	smp_mb();
	printk("%s: @@@@BCM4330 exit resume @@@, func->num=%d\n",__func__,func->num);
	return 0;
}
