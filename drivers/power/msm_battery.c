		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = msm_battery_info.battery_temp*10;
		break;
	default:
		return -EINVAL;
	}
...
if (entry_msm_pm_restart) {
	printk(KERN_ERR "%s: entry_msm_pm_restart %d\n", __func__, entry_msm_pm_restart);
	return 0;
}
...
void msm_battery_update_psy_status(void)
{
	...
	else
	{
		...
		battery_temp		= reply_charger.battery_temp;
		is_charging		= reply_charger.is_charging;
		is_charging_complete	= reply_charger.is_charging_complete;
		is_charging_failed	= reply_charger.is_charging_failed;
	}
...
/*		int rc;
	...
	if (msm_battery_info.batt_handle != INVALID_BATT_HANDLE) {
		rc = msm_batt_modify_client(msm_battery_info.batt_handle,
